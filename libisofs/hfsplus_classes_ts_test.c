/*
cc -g -o demo/classes libisofs/hfsplus_classes_ts_test.c
*/
#include <stdint.h>
#include <string.h>
#include <stdio.h>
...
void make_hfsplus_class_pages()
{
    ...
    break;
        ...
        page_count++;
        if (page_count > 19) {
            fprintf(stderr, "FATAL: More than 19 pages encountered\n");
            exit(1);
        }
    ...
    if (page_count != 19) {
        fprintf(stderr, "FATAL: Less than 19 pages encountered: %d\n",
                page_count);
        exit(1);
    }
}
...
static int compare_class_pages()
{
    ...
    for (i = 0; i < 256; i++) {
        if (hfsplus_class_pages_old[i] == NULL) {
            if (hfsplus_class_pages[i] == NULL)
    continue;
            fprintf(stderr, "DIFF : i= %d , ts != NULL\n", i);
            return 0;
        } else if (hfsplus_class_pages[i] == NULL) {
            fprintf(stderr, "DIFF : i= %d , ts == NULL\n", i);
            return 0;
        }
        for (j = 0; j < 256; j++)
             if (hfsplus_class_pages_old[i][j] !=
                 hfsplus_class_pages[i][j]) {
                 fprintf(stderr, "DIFF : i= %d , j= %d : %4.4x !+ %4.4x\n",
                                 i, j, hfsplus_class_pages[i][j],
                                 hfsplus_class_pages[i][j]);
                     return 0;
                 }
    }
...
main()
{
 ...
 /* hfsplus_class_pages[3][1] = 0xffff; */
 /* hfsplus_class_pages_old[3] = NULL; */
